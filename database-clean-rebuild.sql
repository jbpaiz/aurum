-- ============================================
-- AURUM - LIMPEZA E RECRIAÇÃO COMPLETA DO BANCO
-- ============================================
...
-- Desabilitar políticas RLS temporariamente
...
-- Primeiro remover todas as políticas RLS para evitar erros
DO $$ 
DECLARE 
    r RECORD;
BEGIN
    ...
    -- Desabilitar RLS
    FOR r IN (SELECT tablename FROM information_schema.tables WHERE table_schema = 'public' AND table_name != 'schema_migrations') 
    LOOP
        EXECUTE 'ALTER TABLE IF EXISTS ' || r.tablename || ' DISABLE ROW LEVEL SECURITY';
    END LOOP;
END $$;
...
-- Tabela de provedores de cartão
...
-- Tabela de transações UNIFICADAS (inclui transferências)
...
BEGIN
    ...
    RAISE NOTICE '  ✅ Tabelas: %/7', table_count;
    ...
    IF table_count = 7 AND function_count = 5 THEN
